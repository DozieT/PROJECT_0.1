...
def classify_images(images_dir,results_dic, model):
    ...
    for key in results_dic:
       ...
       # as classifier label
       model2= classifier(images_dir+key, model)
       ...
       if found >= 0:
           if ( (found == 0 and len(truth)==len(model2)) or
                (  ( (found == 0) or (model2[found - 1] == " ") )  and
                   ( (found + len(truth) == len(model2)) or   
                      (model2[found + len(truth): found+len(truth)+1] in 
                     (","," ") ) 
                   )      
                )
              ):
               # found label as stand-alone term (not within label)
               if key not in results_dic1:
                   results_dic1[key] = [truth, model2, 1]
                   
           # found within a word/term not a label existing on its own 
           else:
               if key not in results_dic1:
                   results_dic1[key] = [truth, model2, 0]
